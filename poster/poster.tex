...
%			\end{itemize}
		...
		\begin{block}{Bellman-Ford algorithm~\cite{CLRS}}
			Systems of difference constraints~\subref{subfig:constraints} can be transformed into constraint graphs with an extra vertex~\subref{subfig:graph}. The Bellman-Ford single-source shortest paths algorithm can be used to find a solution from the distances~\subref{subfig:solution}, or detect a conflict from negative weight cycle.
			\vspace*{-1em}
			\begin{figure}[h]
				\centering
